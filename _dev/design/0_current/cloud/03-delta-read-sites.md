...
cache code that don't pass `storage_options`. (The codebase has 44
`scan_delta` and 8 `write_delta` calls total — doc 02 handles the 19
+ 6 inside the storage layer; this doc handles the remaining 25 + 2.) These calls work locally
...
| `visualization/graph/micro.py` | 3 `scan_delta` calls updated: 1 in `_scan_or_empty` helper (called by `_load_executions`, `_load_execution_edges`, `_load_artifact_edges`) + 2 in `_load_artifact_labels`. |
...
- `cloud-deployment.md` — high-level cloud deployment plan (this is Phase 2, step 3)
