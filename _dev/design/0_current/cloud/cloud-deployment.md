...
- **Dispatch is Prefect-coupled.** Both backends construct Prefect
  `@flow` callables inline in their `dispatch()` methods. Cloud backends
  like Modal and Lambda have no Prefect task runner. The interface
  *allows* returning any callable, but the only shared pattern assumes
  Prefect.
...
|-----------|------|----------|
| `RuntimeEnvironment` | `schemas/execution/runtime_environment.py` | 5 `Path` fields (`delta_root_path`, `staging_root_path`, `working_root_path`, `failure_logs_root`, `files_root_path`) |
...
| `DispatchHandle.dispatch()` | `orchestration/backends/{local,slurm}.py` | Constructs Prefect `@flow` inline |
...
**Designed in:** `01-storage-config.md` → `02-storage-layer.md` →
`03-delta-read-sites.md` → `04-runtime-env-rename.md`

Three design principles govern the storage abstraction:

- **One mental model.** All storage operations go through fsspec.
  Local runs use `LocalFileSystem`. Cloud runs use `S3FileSystem` or
  `GCSFileSystem`. Staging, commit, and store access code are the
  same regardless of backend.

- **Libraries do the heavy lifting.** Polars and delta-rs already
  support S3/GCS URIs natively. The framework passes URI strings and
  `storage_options` dicts. No custom I/O code for cloud.

- **Worker sandbox stays local.** Workers always use local `/tmp` for
  their sandbox. Operations never see the storage abstraction.
...
**What does NOT change:** Content-addressed hashing (by content, not
location), cache keys (based on artifact IDs + params, not paths),
...
### Phase 1: DispatchHandle (complete)

Decoupled dispatch from Prefect. The `DispatchHandle` interface is
implemented, and existing Prefect-based backends are wrapped in handles.
This is a prerequisite for cloud backends and independently valuable for
cancellation support.

**Status:** Implemented and merged. `dispatch-handle.md` is archived.

### Phase 2: Storage Abstraction

Introduce fsspec as the unified storage layer. Four sequential PRs,
each independently shippable and testable:

- `01-storage-config.md` — `StorageConfig` foundation + URI utilities
- `02-storage-layer.md` — migrate storage classes to fsspec
- `03-delta-read-sites.md` — thread `storage_options` through 25
  remaining `scan_delta` + 2 `write_delta` calls
- `04-runtime-env-rename.md` — `Path` → `str` field rename (includes
  `files_root_path` → `files_root`, `PipelineConfig.files_root` type
...
- **Return-value staging.** For serverless backends (Modal, Lambda),
  the fsspec path writes to S3 and reads back from S3, adding ~100ms
  per unit in API latency. Workers could instead accumulate DataFrames
  in memory and return them through the dispatch handle, bypassing S3.
  Deferred — the fsspec path is correct, uniform, and testable.

---

## References

- `dispatch-handle.md` (archived) — DispatchHandle interface, state
  machine, backend implementations, cancellation wiring. Phase 1 is
  complete.
- `01-storage-config.md` through `04-runtime-env-rename.md` — storage
  abstraction implementation docs
