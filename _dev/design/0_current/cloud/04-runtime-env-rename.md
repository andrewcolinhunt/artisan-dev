...
`RuntimeEnvironment` carries three path fields as `pathlib.Path`:
...
Frozen Pydantic `BaseModel`. Four path fields: `delta_root_path: Path`,
`staging_root_path: Path`, `working_root_path: Path | None`,
`failure_logs_root: Path | None`. Not persisted to disk — constructed
fresh per pipeline run by `step_executor._create_runtime_environment()`.

### `PipelineConfig` (`schemas/orchestration/pipeline_config.py`)

Frozen Pydantic `BaseModel`. Three path fields: `delta_root: Path`,
`staging_root: Path`, `working_root: Path`. Note: already uses the
target names (no `_path` suffix), but types are still `Path`.
...
| `orchestration/engine/step_executor.py` | `delta_root_path`, `staging_root_path`, `working_root_path` |
| `execution/executors/creator.py` | `delta_root_path`, `staging_root_path`, `working_root_path` |
| `execution/executors/curator.py` | `delta_root_path`, `staging_root_path` |
| `execution/executors/composite.py` | `delta_root_path`, `staging_root_path`, `working_root_path` |
...
| File | Fields accessed |
|------|----------------|
| `orchestration/engine/step_executor.py` | `config.delta_root`, `config.staging_root`, `config.working_root` |
| `orchestration/pipeline_manager.py` | `config.delta_root`, `config.staging_root` |
...
### `working_root` and `failure_logs_root` — local path handling

These paths are always local. Consumers that need `Path` operations
wrap them:
...
- `cloud-storage-design.md` — full storage design (this is Phase 4)
...
- `cloud-deployment.md` — after this PR, the codebase is cloud-ready
