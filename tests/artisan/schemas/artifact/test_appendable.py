...
def _draft_record(record: dict, external_path: str) -> AppendableArtifact:
    """Create a draft matching the producer's exact record bytes."""
    line = json.dumps(record, sort_keys=True).encode()
    return AppendableArtifact.draft(
        record_id=record["record_id"],
        content_hash=compute_content_digest(line),
        size_bytes=len(line),
        step_number=1,
        external_path=external_path,
        original_name=record["record_id"],
    )
...
class TestFinalize:
    ...
    def test_different_record_id_different_artifact_id(self) -> None:
        ...
        art_a = AppendableArtifact.draft(
            record_id="a",
            content_hash=compute_content_digest(line_a.encode()),
        ...
        art_b = AppendableArtifact.draft(
            record_id="b",
            content_hash=compute_content_digest(line_b.encode()),
    ...
    def test_same_record_different_path_same_id(self) -> None:
        """Moving a record to another container preserves its ID."""
...
class TestReadRecordAutoInferFs:
    ...
    def test_auto_infers_memory_fs_from_uri(self, tmp_path: Path) -> None:
        ...
        art = _draft_record(
            records[1], "memory:///test_appendable/auto/data.jsonl"
        ).finalize()
    ...
    def test_explicit_fs_overrides_inference(self, tmp_path: Path) -> None:
        ...
        art = _draft_record(
            {"record_id": "rec_000000", "x": 7}, str(jsonl_path)
        ).finalize()
