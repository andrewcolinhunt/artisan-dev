...
def register(mcp: FastMCP) -> None:
    ...
    async def artisan_get_run_status(ctx: Context, pipeline_run_id: str) -> dict:
        ...
        ended_at) plus a per-step list of terminal statuses (ok, skipped,
        cancelled, failed) with a produced summary and duration. "Running"
        is never persisted, so an in-flight step shows only its last
        recorded terminal event. Use this to watch progress or locate a
        failed step, then artisan_get_step_logs or artisan_diagnose_run to
        dig in. An unknown run yields empty steps, not an error.
        """
        config = ctx.lifespan_context["config"]
